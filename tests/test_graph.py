...
class GraphTestCase(unittest.TestCase):
    ...
    def test_graph_duplicate_add_edge(self):
        ...
        self.g.add_edge(
            self.n1, self.n2
        )  # testing self.n2, self.n1 shouldn't change anything, see Graph
        self.assertEqual(self.g.node_count, x, "Graph's node_count changed")
    ...
    def test_graph_add_node(
        ...
        self.assertRaises(ValueError,
                          "Should raise error b/c node already in graph")
    ...
    def test_graph_new_node_add_edge(self):
        self.g.add_edge(
            self.n1, self.n4
        )  # if new node added by edge, should be added to list_nodes? (Currently isn't)
        self.assertEqual(
            self.g.node_count,
            4)  # No, because that's how clusters work - not our def of graph
    ...
    def test_graph_delete_edge(self):
        ...
        self.g.delete_edge(
            self.n1,
            self.n2)  # List.remove() throws error if remove non existing
        self.assertEqual(self.g.node_count, 3,
                         "Node_count for graph incorrect")
        ...
        self.assertNotIn(self.n1, self.n2.edges,
                         "All instances not removed from adj list")
    ...
    def test_graph_delete_node(self):
        ...
        self.assertEqual(self.g.list_nodes, [self.n2, self.n3],
                         "Node not deleted")
    ...
    def test_delete_node_delete_edge(self):
        self.g.add_edge(
            self.n2,
            self.n1)  # g initialized w/n1 and n2, but no connecting edges
    ...
    def test_eq_wrong_positions(self):
        ''' Tests the eqals method with graphs that have the
        ...
        self.assertTrue(
            (self.g != second_graph),
            "Graphs with nodes in d/f positions shouldn't be considerd equal")

    def test_eq_identical(self):
        ''' Happy path: tests the same exact graphs are considerd equal'''
