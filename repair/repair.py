...
from nodes.nodes import RepairNode


class RepairPriorityQueue(PriorityQueue):
    ''' A priority queue implementation
    Overrides a few methods to make python's queue implementation to
    work the way we need it to

    .. note:: Both put and get default block to False instead of true.
              This makes the implementation closer to put_nowait
              and get_nowait. It makes a difference in a multithreaded setup
    '''
    ...
    def put(self, item, block=False, timeout=None):
        ...
        Get the pair from the dictionary which contains the same
        number of items. If it finds the time, update the freq
        before inserting. Otherwise put into the queue '''
...
class Repair:
    ...
    def compress(self):
        ...
        # TODO move this to graph, use events
        for node in self.graph.list_nodes:
